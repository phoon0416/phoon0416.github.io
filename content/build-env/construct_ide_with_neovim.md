...
2. 如果你需要使用cscope插件，首先在主机上通过sudo apt install cscope命令安装cscope，***然后进入到工程源码根目录***，按照以下步骤生成工程的cscope.out文件
```bash
find . -name "*.c" -o -name "*.h" -o -name "*.S" -o -name "*.s" -o -name "*.hpp" -o -name "*.cpp" -o -name "*.cc" > cscope.files
